...
function ReservoirData(props) {
    ...
    //         <ComposedChart className="line-chart" data={(chartRange === '1 year' ? props.data.slice(props.data.length - 12, props.data.length) : chartRange === '5 years' ? props.data.slice(props.data.length - 60, props.data.length) : props.data.slice(0, props.data.length))} margin={{ top: 0, right: window.innerWidth < 868 ? 20 : 40, left: window.innerWidth < 868 ? 10 : 50, bottom: 20 }}>
    ...
    //             <XAxis dataKey="date" interval={(chartRange === '1 year' ? 0 : chartRange === '5 years' ? 2 : 11)} tick={{ fontSize: window.innerWidth < 868 ? 8 : 12, angle: window.innerWidth < 868 ? -90 : -20, dy: window.innerWidth < 868 ? 16 : 25 }} />
